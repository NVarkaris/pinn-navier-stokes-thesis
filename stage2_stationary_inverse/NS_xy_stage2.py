"""
Stage 2: Stationary inverse PINN for incompressible Navier-Stokes flow.

The model is trained using physics-informed losses and sparse pressure
measurements extracted from the COMSOL reference solution.

The inverse problem estimates the inlet pressure p_in.

COMSOL data are used for pressure observation extraction, evaluation, and
plotting. They are not used as full-field supervised training data.
...
PLOT_DATA_DIR = REPO_ROOT / "stage2_stationary_inverse"
FIGURES_DIR = REPO_ROOT / "stage2_stationary_inverse" / "figures"
...
if DATA_POINTS not in (2, 3, 4, 5):
    raise SystemExit("Invalid amount of pressure data points.")
...
class PINN(nn.Module):
    def __init__(self, layers, p_in_initial_dimless):
        super().__init__()
        ...
        self.p_in_param = nn.Parameter(
            torch.tensor([p_in_initial_dimless], dtype=torch.float32)
        )
...
model = PINN(layers, p_in_initial / p_ref).to(device)
...
def closure():
    ...
    p_in_history_lbfgs.append(model.p_in_param.item() * p_ref)
...
# =========================
# Load COMSOL reference data for pressure observations and evaluation
# =========================
...
# =========================
# Pressure observation points on y = H/2
# =========================
...
x_obs_phys = np.array(
    observation_scenarios[DATA_POINTS],
    dtype=float
).reshape(-1, 1)

y_obs_phys = np.full_like(x_obs_phys, y_obs_val)

p_obs_phys = griddata(
    np.hstack([x_dim, y_dim]),
    p_dim.flatten(),
    np.hstack([x_obs_phys, y_obs_phys]),
    method="linear"
).reshape(-1, 1)
...
x_print = x_obs_phys.flatten()
p_print = p_obs_phys.flatten()

print(f"Using {DATA_POINTS} pressure measurements at x={fmt_vec(x_print)} m, y={fmt_num(y_obs_val)} m")
print(f"Measured p (Pa): {fmt_vec(p_print)}")
print("=" * 60)
...
def loss_function(return_diag=False):
    ...
    loss_in = torch.mean((p_in_pred - model.p_in_param)**2 + u_y_in_pred**2)
    ...
    # ---- Pressure observation data loss calculation ----
    output_obs = model(y_obs, x_obs)

    p_obs_pred = output_obs[:, 2:3]

    loss_data = torch.mean((p_obs_pred - p_obs)**2)
...
RUN_NAME = f"pinit{num_to_tag(p_in_initial)}_points{DATA_POINTS}"
CHECKPOINT_PATH = MODEL_DIR / f"stage2_inverse_{RUN_NAME}.pt"
HISTORY_PATH = HISTORY_DIR / f"stage2_{RUN_NAME}.pt"
PLOT_DATA_PATH = PLOT_DATA_DIR / f"stage2_{RUN_NAME}_plot_data.npz"
...
def expected_figure_paths():
    figure_names = [
        ...
        "vertical_three_panel_vmag_stationary_inverse",
        "vertical_three_panel_pressure_stationary_inverse",
    ]

    return [RUN_FIGURES_DIR / f"{sanitize_filename(name)}.png" for name in figure_names]

def check_existing_outputs_before_run():
    if TRAIN_MODE and SAVE_CHECKPOINT:
        stop_if_output_exists(CHECKPOINT_PATH)

    if TRAIN_MODE and SAVE_HISTORY:
        stop_if_output_exists(HISTORY_PATH)
...
def save_stage2_checkpoint(p_in_after_adam, p_in_after_lbfgs):
    checkpoint = {
        ...
        "inverse_problem": {
            "estimated_parameter": "p_in",
            "p_in_initial": p_in_initial,
            "p_in_reference": p_in_dim,
            "data_points": DATA_POINTS,
            "x_obs_phys": x_obs_phys.flatten().tolist(),
            "y_obs_val": y_obs_val,
            "p_obs_phys": p_obs_phys.flatten().tolist(),
            "p_in_after_adam": p_in_after_adam,
            "p_in_after_lbfgs": p_in_after_lbfgs,
        },
        ...
        "p_in_history": p_in_history,
        "p_in_history_lbfgs": p_in_history_lbfgs,
    }
...
def save_stage2_history(p_in_after_adam, p_in_after_lbfgs, adam_time, lbfgs_time, optimizer_time, overhead_time, total_time):
    history_checkpoint = {
        "run_name": RUN_NAME,
        "data_points": DATA_POINTS,
        "p_in_initial": p_in_initial,
    ...
    torch.save(history_checkpoint, HISTORY_PATH)

    print("\n" + "=" * 60)
    print(f"Saved p_in history: {HISTORY_PATH}")
    print("=" * 60)
...
if TRAIN_MODE:
    ...
    p_in_history.append(model.p_in_param.item() * p_ref)
    ...
    print("\n--- Inverse parameter ---")
    print(f"[INV] p_in = {model.p_in_param.item() * p_ref:.4f} Pa")
    ...
    p_in_after_adam = model.p_in_param.item() * p_ref
    pct_dev_adam = 100.0 * abs(p_in_after_adam - p_in_dim) / abs(p_in_dim)
    print(f"\n[INV] p_in after Adam   = {p_in_after_adam:.4f} Pa | deviation = {pct_dev_adam:.4f}%")
    ...
    p_in_after_lbfgs = model.p_in_param.item() * p_ref
    pct_dev_lbfgs = 100.0 * abs(p_in_after_lbfgs - p_in_dim) / abs(p_in_dim)
    print(f"[INV] p_in after L-BFGS = {p_in_after_lbfgs:.4f} Pa | deviation = {pct_dev_lbfgs:.4f}%")
    ...
    if SAVE_CHECKPOINT:
        save_stage2_checkpoint(p_in_after_adam, p_in_after_lbfgs)
    ...
    p_in_history = checkpoint.get("p_in_history", [])
    p_in_history_lbfgs = checkpoint.get("p_in_history_lbfgs", [])

    inverse_problem = checkpoint.get("inverse_problem", {})
    p_in_after_adam = inverse_problem.get("p_in_after_adam", model.p_in_param.item() * p_ref)
    p_in_after_lbfgs = inverse_problem.get("p_in_after_lbfgs", model.p_in_param.item() * p_ref)
...
print("-" * 60)
print(f"Mean predicted p_inlet (Pa) :   {torch.mean(p_in_pred).item():.6e}")
print(f"Mean inferred p_in (Pa)     :   {model.p_in_param.item() * p_ref:.6e}")
print(f"Mean predicted p_outlet (Pa):   {torch.mean(p_out_pred).item():.6e}")
print(f"Reference p_inlet (Pa)      :   {p_in_dim:.6e}")
print(f"Reference p_outlet (Pa)     :   {p_out_dim:.6e}")
print("-" * 60)
...
if SAVE_PLOT_DATA:
    ...
    np.savez(
        ...
        p_in_after_adam=p_in_after_adam,
        p_in_after_lbfgs=p_in_after_lbfgs,
        p_in_history=np.asarray(p_in_history),
        p_in_history_lbfgs=np.asarray(p_in_history_lbfgs),

        x_obs_phys=x_obs_phys,
        y_obs_phys=y_obs_phys,
...
if CREATE_PLOTS:
    ...
    if len(p_in_history) > 0:

        fig, ax = plt.subplots(figsize=(10, 6))

        n_adam = len(p_in_history)
        ...
        if len(p_in_history_lbfgs) > 0:
            lbfgs_x = range(n_adam - 1, n_adam + len(p_in_history_lbfgs))
            lbfgs_y = [p_in_history[-1]] + p_in_history_lbfgs

            ax.plot(
                lbfgs_x,
                lbfgs_y,
                linewidth=1.8,
                label=f"p_in (L-BFGS) -> {p_in_after_lbfgs:.3f} Pa"
            )

        ax.axhline(p_in_dim, linestyle="--", label=f"Reference = {p_in_dim} Pa")

        ax.set_xlabel("Training step")
        ax.set_ylabel("p_in (Pa)")
        ax.set_title("Inlet pressure inference")
        ax.grid(alpha=0.3)
        ax.legend(loc="best")

        plt.tight_layout()
        save_fig(fig, "p_in_evolution")
        show_or_close(fig)
    ...
    plot_three_panel_vertical(
        save_name="vertical_three_panel_vmag_stationary_inverse",
        field_symbol=r"|v|",
        unit="m/s",
        comsol_values=v_mag_data_np,
        pinn_values=v_mag_pred_np,
    ...
    plot_three_panel_vertical(
        save_name="vertical_three_panel_pressure_stationary_inverse",
        field_symbol=r"$p$",
        unit="Pa",
        comsol_values=p_data_np,
        pinn_values=p_pred_np,
